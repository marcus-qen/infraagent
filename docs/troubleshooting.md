...
**Symptom:** `kubectl get legators` shows `Phase: Pending`

**Cause:** The referenced LegatorEnvironment doesn't exist.

**Fix:**
```bash
# Check the condition
kubectl describe legator <name> -n <namespace>
...
**Symptom:** Agent is `Ready` but no LegatorRuns are created.

**Checks:**
1. **Leader election**: Only the leader schedules runs. Check controller logs:
   ```bash
   kubectl logs -n legator-system deploy/legator-controller | grep "leader"
   ```
2. **Paused**: Check `spec.paused`:
   ```bash
   kubectl get legator <name> -o jsonpath='{.spec.paused}'
   ```
3. **Rate limited**: Check controller logs for "rate-limited"
4. **Cron expression**: Validate with `crontab.guru`

## LegatorRun Failed — "assembly failed"

**Symptom:** LegatorRun with phase=Failed, report says "assembly failed"
...
**Fix:**
```bash
# Check the LegatorRun for details
...
## LegatorRun Failed — "LLM call failed"

**Symptom:** LegatorRun Failed, report mentions LLM errors.
...
kubectl logs -n legator-system deploy/legator-controller | grep "LLM"

# Check network policy isn't blocking egress
kubectl get networkpolicy -n legator-system
```

## All actions blocked

**Symptom:** LegatorRun with phase=Blocked, all actions have status=blocked.
...
kubectl get agentruns -n <namespace> -l legator.io/agent=<name> \
...
## LegatorRuns accumulating

**Symptom:** Many old LegatorRun CRs consuming etcd storage.
...
kubectl logs -n legator-system deploy/legator-controller --previous

# Check events
kubectl get events -n legator-system --sort-by=.lastTimestamp

# Check resource limits
kubectl describe pod -n legator-system -l app.kubernetes.io/component=controller
```
...
1. Check controller logs: `kubectl logs -n legator-system deploy/legator-controller`
2. Check LegatorRun details: `kubectl describe agentrun <name> -n <namespace>`
3. Check LegatorAgent conditions: `kubectl describe legator <name> -n <namespace>`
4. File an issue: [github.com/marcus-qen/legator/issues](https://github.com/marcus-qen/legator/issues)
