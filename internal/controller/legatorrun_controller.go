...
// LegatorRunReconciler reconciles an LegatorRun object.
type LegatorRunReconciler struct {
	client.Client
...
func (r *LegatorRunReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	log.Info("Reconciling LegatorRun",
		"name", run.Name,
	...
	// - Update parent LegatorAgent status (lastRunTime, runCount, consecutiveFailures)
	// - Handle retention/cleanup of old LegatorRuns

	return ctrl.Result{}, nil
}

// SetupWithManager sets up the controller with the Manager.
func (r *LegatorRunReconciler) SetupWithManager(mgr ctrl.Manager) error {
	return ctrl.NewControllerManagedBy(mgr).
		For(&corev1alpha1.LegatorRun{}).
		Named("agentrun").
		Complete(r)
}
