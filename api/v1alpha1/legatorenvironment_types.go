...
// LegatorEnvironmentSpec defines the site-specific configuration for an agent.
type LegatorEnvironmentSpec struct {
...
// LegatorEnvironmentPhase represents the lifecycle phase.
// +kubebuilder:validation:Enum=Pending;Ready;Error
type LegatorEnvironmentPhase string

const (
	LegatorEnvironmentPhasePending LegatorEnvironmentPhase = "Pending"
	LegatorEnvironmentPhaseReady   LegatorEnvironmentPhase = "Ready"
	LegatorEnvironmentPhaseError   LegatorEnvironmentPhase = "Error"
)

// LegatorEnvironmentStatus defines the observed state of LegatorEnvironment.
type LegatorEnvironmentStatus struct {
	// phase is the current lifecycle phase.
	// +optional
	Phase LegatorEnvironmentPhase `json:"phase,omitempty"`
...
// LegatorEnvironment is the Schema for the agentenvironments API.
...
// that an LegatorAgent binds to at runtime.
type LegatorEnvironment struct {
	metav1.TypeMeta   `json:",inline"`
	metav1.ObjectMeta `json:"metadata,omitempty"`

	// +required
	Spec   LegatorEnvironmentSpec   `json:"spec"`
	Status LegatorEnvironmentStatus `json:"status,omitempty"`
}

// +kubebuilder:object:root=true

// LegatorEnvironmentList contains a list of LegatorEnvironment.
type LegatorEnvironmentList struct {
	metav1.TypeMeta `json:",inline"`
	metav1.ListMeta `json:"metadata,omitempty"`
	Items           []LegatorEnvironment `json:"items"`
}

func init() {
	SchemeBuilder.Register(&LegatorEnvironment{}, &LegatorEnvironmentList{})
}
